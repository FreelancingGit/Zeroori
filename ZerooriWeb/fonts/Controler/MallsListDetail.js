angular.module("ZerooriApp", ['ngCookies']).controller("MallsListDetail", function ($scope, $http, $cookies) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'mallsdetail';
    ...
    $scope.MallAdMastId = $cookies.get('MADID');

    $http({
        method: 'POST',
        url: 'MallsListDetail.ashx',
        params: {
        ...
        if ($scope.isValidSave(response)) {
            ...
            $scope.FilterData.MallMast.MallAdMastId = $cookies.get('MADID');
        ...
        try {
            $http({
                ...
                if ($scope.isValidSave(response)) {
                    ...
                    $scope.MallMast = response.data.MallMast;
