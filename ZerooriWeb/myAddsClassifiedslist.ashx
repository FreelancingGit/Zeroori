<%@ WebHandler Language="C#" Class="myAddsClassifiedslist" %>
...
public class myAddsClassifiedslist : IHttpHandler
{
    #region Properties
    ...
    private void doLoad()
    {
        ...
        if (Key != "")
        {
            ...
            else if (Key == "Init")
            {
                ...
                Mode = "LO";
                ZA3720myaddsM UserPrflM = new ZA3720myaddsM();
                UserVew = UserPrflM.DoInit(UserFltr, Mode);
                this.Context.Response.Write(ObjectToJson(UserVew));
            }
             else if (Key == "DeleteProduct")
            {
                ZA3620SD UserFltr = JsonToObject<ZA3620SD>(JsonData);
                ZA3720ILD UserVew = new ZA3720ILD();

                ZA3720myaddsM UserPrflM = new ZA3720myaddsM();
                UserVew = UserPrflM.DeleteProduct(UserFltr);
                this.Context.Response.Write(ObjectToJson(UserVew));
            }
