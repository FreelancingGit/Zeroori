<%@ WebHandler Language="C#" Class="PropertiesList" %>
...
public class PropertiesList : IHttpHandler
{
    #region Properties
    ...
    private void doLoad()
    {
        ...
        if (Key != "")
        {
            ...
            else if (Key == "Init")
            {
                ...
                Mode = "LO";
                ZA3710M UserPrflM = new ZA3710M();
                UserVew = UserPrflM.DoInit(UserFltr, Mode);
                this.Context.Response.Write(ObjectToJson(UserVew));
            }
            else if (Key == "LoadData")
            {
                ZA3710LFD UserFltr = JsonToObject<ZA3710LFD>(JsonData);
                ZA3710ILD UserVew = new ZA3710ILD();
