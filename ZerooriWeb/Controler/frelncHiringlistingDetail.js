angular.module("ZerooriApp", ['ngCookies']).controller("frelncHiringlistingDetail", function ($scope, $http, $cookies) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'frelnc-hiring-detail';
    ...
    $http({
        method: 'POST',
        url: 'frelncshiringlistingdetail.ashx',
        params: {
