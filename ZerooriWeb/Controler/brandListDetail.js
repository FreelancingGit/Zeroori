angular.module("ZerooriApp", ['ngCookies']).controller("brandListDetail", function ($scope, $http, $cookies, $filter) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'brandslist-detail';
    ...
    $http({
        ...
        if ($scope.isValidSave(response)) {
            ...
            //$scope.FilterData.PackDealMast.PackDealMastId = $cookies.get('DEALID');
        ...
        try {
            $http({
                ...
                if ($scope.isValidSave(response)) {
                    ...
                    $scope.PackDealMast = response.data.PackDealMast;
    ...
    $scope.GetViewDeal = function (DealMastId) {
