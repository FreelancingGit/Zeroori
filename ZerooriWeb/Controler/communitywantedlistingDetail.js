angular.module("ZerooriApp", ['ngCookies']).controller("communitywantedlistingDetail", function ($scope, $http, $cookies) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'community-wanted-listing-detail';
    ...
    $scope.EmpJobMastID = $cookies.get('MADID');


    $http({
        method: 'POST',
        url: 'jobswantedlistingdetails.ashx',
        params: {
        ...
        if ($scope.isValidSave(response)) {
            ...
            $scope.FilterData.EmpJobMast.EmpJobMastID = $cookies.get('MADID');
        ...
        try {
            $http({
                ...
                if ($scope.isValidSave(response)) {
                    ...
                    $scope.EmpJobMast = response.data.EmpJobMast;
                } // User Login Mode
    ...
    $scope.LoadData = function () {
        ...
        try {
            $http({
                ...
                url: 'jobswantedlistingdetails.ashx',
                params: {
                    CLoadData: JSON.stringify($scope.SelectedData)
                }
            }).then(function successCallback(response) {
