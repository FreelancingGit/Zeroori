angular.module("ZerooriApp", ['ngCookies']).controller("DirctoryListDetail", function ($scope, $http, $cookies) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'directorylistdetail';
    ...
    $scope.FilterData.DirMast.DirDtlId = $cookies.get('DIRID');

    $http({
        method: 'POST',
        url: 'DealsListDetail.ashx',
        params: {
        ...
        try {
            $http({
                ...
                if ($scope.isValidSave(response)) {
                    ...
                    $scope.DirMast = response.data.DirMast;
                    $scope.isLoading = false;
                } // User Login Mode
