angular.module("ZerooriApp", ['ngCookies']).controller("myAddsClassifiedList", function ($scope, $http, $cookies) {
	$scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'myAdds-classifiedslist';
	...
	$http({
		method: 'POST',
        url: 'myAddsClassifiedslist.ashx',
		params: {
    ...
    $scope.DeleteProduct = function (ClasifdAdMastId) {
        $scope.SelectedData.ClasifdAdMastId = ClasifdAdMastId;
        if (($scope.SessionId != null && $scope.SessionId != "" && ClasifdAdMastId != null)) {
            try {
                $http({
                    method: 'POST',
                    url: 'myAddsClassifiedslist.ashx',
                    params: {
                        DeleteProduct: JSON.stringify($scope.SelectedData)
                    }
                }).then(function successCallback(response) {
    ...
    $scope.UpdateClassfds = function (ClassifdAdMastId) {
        $cookies.put("CFID", ClassifdAdMastId);
        $scope.navigate("classifiedsstepone");
	}
