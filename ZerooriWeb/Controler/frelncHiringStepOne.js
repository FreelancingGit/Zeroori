angular.module("ZerooriApp", ['ngCookies']).controller("frelncHiringStepOne", function ($scope, $http, $cookies) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'frelnc-hiring-step-one';
    ...
    $http({
        ...
        if ($scope.isValidSave(response)) {
            ...
            $scope.SelectedData.UserData.ZaBase.PKID = $cookies.get('FH');
            $scope.SelectedData.CompnyJobMastId = $cookies.get('FH');
            $scope.LoadInit();
        }
                ...
                $http({
                    ...
                    if ($scope.isValidSave(response) ) {
                       ...
                       // alert('You are Successfully Posted');
                        $scope.navigate("frelnc-hiring-step-two");
