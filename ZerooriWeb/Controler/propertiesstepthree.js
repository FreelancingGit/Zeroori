angular.module("ZerooriApp", ['ngCookies']).controller("propertiesstepthree", function ($scope, $http, $cookies) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'propertiesstepthree';
    ...
    $http({
        ...
        if ($scope.isValidSave(response)) {
            ...
            $scope.ViewData.AddPropADMastID = $cookies.get('POID');
            $scope.ViewData.UserData.ZaBase.PKID = $cookies.get('PRID');
            ...
            $http({
                ...
                if ($scope.isValidSave(response)) {
                    ...
                    if (response.data.Price != null ||
                        ...
                        $scope.ViewData.Price = response.data.Price;
                        $scope.ViewData.Location = response.data.Location;
