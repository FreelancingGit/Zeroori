angular.module("ZerooriApp", ['ngCookies']).controller("DealsListDetail", function ($scope, $http, $cookies) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'deals-list-detail';
        ...
        try {
            $http({
                ...
                if ($scope.isValidSave(response)) {
                    ...
                    $scope.DealMast = response.data.DealMast;
                    $scope.DealM = response.data.DealM;
