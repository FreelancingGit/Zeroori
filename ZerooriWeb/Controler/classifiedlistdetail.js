angular.module("ZerooriApp", ['ngCookies']).controller("classifiedlistdetail", function ($scope, $http, $cookies) {
$scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
$scope.Page = 'classifiedslistdetail';
...
$http({
method: 'POST',
url: 'classifiedslistdetail.ashx',
params: {
GetC: JSON.stringify($scope.ClasifdData)
}
...
$scope.ZaKey = response.data.UserData.ZaBase.ZaKey;
    ...
    $scope.ClasifdData.UserData.ZaBase.SessionId = $cookies.get($scope.ZaKey);
$scope.MADID = $cookies.get('CLAID');
...
$scope.ClasifdData.ClasifdAdMastId = $scope.MADID;
try {
$http({
method: 'POST',
url: 'classifiedslistdetail.ashx',
params: {
Init: JSON.stringify($scope.ClasifdData)
}
}).then(function successCallback(response) {
if ($scope.isValidSave(response)) {
    if (response.data.UserData != undefined && response.data.UserData.FistNam != "") {
        ...
        $scope.SetStatus(false);
$scope.ClasifdData = response.data;
...
url: 'classifiedslistdetail.ashx',
params: {
SignOut: JSON.stringify($scope.ClasifdData)
}
}).then(function successCallback(response) {
if ($scope.isValidSave(response)) {
