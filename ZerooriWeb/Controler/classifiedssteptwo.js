angular.module("ZerooriApp", ['ngCookies']).controller("classifiedssteptwo", function ($scope, $http, $cookies) {
    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'classifiedssteptwo';
    ...
    $http({
        ...
        if ($scope.isValidSave(response)) {
            ...
            $scope.ViewData.UserData.ZaBase.PKID = $cookies.get('CFID');
            $scope.ViewData.AddclasifdADMastID = $cookies.get('FlC');
