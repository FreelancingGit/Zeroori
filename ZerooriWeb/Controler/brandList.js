angular.module("ZerooriApp", ['ngCookies']).controller("brandList", function ($scope, $http, $cookies,$filter) {

    $scope.urlArray = window.location.href.slice(window.location.href.indexOf('?') + 1).split('&');
    $scope.Page = 'brandslist';
    ...
    $scope.ShowItemDetail = function (PackDealMastID) {

        $cookies.put("DEALID", PackDealMastID);
        $scope.navigate("brandslist-detail");
    }
