<%@ WebHandler Language="C#" Class="classifiedslistdetail" %>
...
public class classifiedslistdetail : IHttpHandler
{
    #region Properties
    ...
    private void doLoad()
    {
        ...
        if (Key != "")
        {
            ...
            if (Key == "Init")
            {
                ZA3721LD UserFltr = JsonToObject<ZA3721LD>(JsonData);
                ZA3721LD UserVew = new ZA3721LD();

                Mode = "LD";
                ZA3721M UserPrflM = new ZA3721M();
                UserVew = UserPrflM.DoLoad(UserFltr, Mode);
                this.Context.Response.Write(ObjectToJson(UserVew));
            }
            else if (Key == "SignOut")
