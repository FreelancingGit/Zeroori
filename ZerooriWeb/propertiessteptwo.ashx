<%@ WebHandler Language="C#" Class="propertiessteptwo" %>
...
public class propertiessteptwo : IHttpHandler
{
    #region Properties
    ...
    private void doLoad()
    {
        ...
        if (Key != "")
        {
            ...
            if (Key == "LoadUser")
            {
                ...
                ZA3610LD UserVew = new ZA3610LD();

                Mode = "LO";
                ZA3610M UserPrflM = new ZA3610M();
                UserVew = UserPrflM.DoLoad(UserFltr, Mode);
                this.Context.Response.Write(ObjectToJson(UserVew));
            }
            if (Key == "DoSave")
            {
                ZA3610SD UserFltr = JsonToObject<ZA3610SD>(JsonData);
                ZA3610LD UserVew = new ZA3610LD();
