<%@ WebHandler Language="C#" Class="motorssteptwo" %>
...
public class motorssteptwo : IHttpHandler
{
    #region Properties
    ...
    private void doLoad()
    {
        ...
        if (Key != "")
        {
            ...
            if (Key == "LoadUser")
            {
                ...
                ZA3600LD UserVew = new ZA3600LD();

                Mode = "LO";
                ZA3600M UserPrflM = new ZA3600M();
                UserVew = UserPrflM.DoLoad(UserFltr, Mode);
                this.Context.Response.Write(ObjectToJson(UserVew));
            }
            if (Key == "DoSave")
            {
                ZA3600SD UserFltr = JsonToObject<ZA3600SD>(JsonData);
                ZA3600LD UserVew = new ZA3600LD();
